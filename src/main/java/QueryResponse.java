...
public class QueryResponse
{
    ...
    public QueryResponse(byte[] data, boolean fullstat)
    {
        ...
        data = query.ByteUtils.trim(data);
        byte[][] temp = query.ByteUtils.split(data);

//		if(temp.length == 6) //short stat
        if(!fullstat)
        {
            motd			= new String(query.ByteUtils.subarray(temp[0], 1, temp[0].length-1));
            ...
            port			= query.ByteUtils.bytesToShort(temp[5]);
            hostname		= new String(query.ByteUtils.subarray(temp[5], 2, temp[5].length-1));
        }
        else //full stat
        {
